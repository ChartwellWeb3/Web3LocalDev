...
namespace Chartwell.Feature.GeneralContact.Models
{
  public class GeneralContactModel
  {
    ...
    //[RegularExpression(@"^([0-9\s])$", ErrorMessage = "Numbers and space only")]
    //[RegularExpression(@"^?!(\+\s?)?((?<!\+.*)\(\+?\d+([\s\-\.]?\d+)?\)|\d+)([\s\-\.]?(\(\d+([\s\-\.]?\d+)?\)|\d+))*(\s?(x|ext\.?)\s?\d+)?$", ErrorMessage = "The PhoneNumber is not valid")]
